...
import { generatePDF, estimatePDFGenerationTime } from '@/lib/export/pdf-generator';
...
/** Escapes user/AI-provided text for safe HTML interpolation (XSS protection). */
...
function designNodeToHTML(node: DesignNode, indent: number = 0): string {
  ...
  if (node.type === 'text' || node.type === 'heading') {
    ...
    if (!children.length) {
      return `${spaces}<${tag}${styleStr}>${escapeHtml(textContent)}</${tag}>`;
    }
  }

  // Button with content
  if (node.type === 'button' && node.content && !children.length) {
    return `${spaces}<${tag}${styleStr}>${escapeHtml(node.content)}</${tag}>`;
  }
  ...
  try {
    ...
    switch (format) {
      ...
      case 'pdf': {
        ...
        // Wrap Buffer in Uint8Array (zero-copy view): NextResponse's BodyInit type
        // doesn't accept Node's Buffer even though they're binary-compatible at runtime.
        const pdfBytes = new Uint8Array(pdfResult.pdfBuffer);
