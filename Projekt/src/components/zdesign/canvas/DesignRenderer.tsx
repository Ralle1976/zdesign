...
const NodeRenderer: React.FC<NodeRendererProps> = React.memo(
  function NodeRenderer({ node, selectedId, hoveredId, onSelect, onHover, depth }) {
    ...
    // Image nodes are excluded: they carry content as a background-image and must
    // take the dedicated image branch below (an <img> ignores background-image).
    if (isSelfClosingTag(actualTag) && node.type !== 'image') {
...
// Wraps each node (including every recursive child) in an ErrorBoundary so a
// single malformed LLM node renders a small placeholder instead of blanking
// the whole canvas. resetKeys=[node.id] auto-recovers when the node changes.

function NodeErrorFallback({ error }: FallbackProps) {
  const firstLine = (error?.message || 'render error').split('\n')[0].slice(0, 64);
  return (
    <div
      role="alert"
      className="inline-flex items-center gap-1.5 rounded-md border border-amber-300/50 bg-amber-50 px-2 py-1 text-[10px] font-medium text-amber-700 dark:border-amber-700/50 dark:bg-amber-900/20 dark:text-amber-300"
      title={error?.message}
    >
      <AlertTriangle className="size-3 shrink-0" aria-hidden="true" />
      <span className="truncate">Malformed node: {firstLine}</span>
    </div>
  );
}

const SafeNodeRenderer: React.FC<NodeRendererProps> = function SafeNodeRenderer(props) {
  return (
    <ErrorBoundary fallback={NodeErrorFallback} resetKeys={[props.node.id]}>
      <NodeRenderer {...props} />
    </ErrorBoundary>
  );
};
