...
// Z.Design - Property Editors + PropertiesContent
//
// Extracted from PropsPanel.tsx (T6 modularization, 2026-07-04).
// Contains: helper functions, shared sub-components (PropLabel, ColorInput,
// IconButton), the seven accordion editors (Content, Layout, Spacing,
// Typography, Background, Border, Effects), and PropertiesContent which
// composes them. PropsPanel.tsx now only owns the tab shell and imports
// PropertiesContent from here.
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  onChange={(e) => onStyleChange('gap', numToPx(e.target.value))}
                  placeholder="0"
      ...
      <AccordionContent>
        <div className="space-y-3 pt-1">
          ...
          <div>
            ...
            <div className="flex gap-1">
              <IconButton active={style.textAlign === 'left' || !style.textAlign} onClick={() => onStyleChange('textAlign', 'left')} title="Align Left">
                <AlignLeft className="size-3.5" />
              </IconButton>
              <IconButton active={style.textAlign === 'center'} onClick={() => onStyleChange('textAlign', 'center')} title="Align Center">
                <AlignCenter className="size-3.5" />
              </IconButton>
              <IconButton active={style.textAlign === 'right'} onClick={() => onStyleChange('textAlign', 'right')} title="Align Right">
                <AlignRight className="size-3.5" />
              </IconButton>
              <IconButton active={style.textAlign === 'justify'} onClick={() => onStyleChange('textAlign', 'justify')} title="Justify">
          ...
          <div>
            ...
            <ColorInput value={style.color || ''} onChange={(hex) => onStyleChange('color', hex)} />
          </div>
          <div>
      ...
      <AccordionContent>
        <div className="space-y-3 pt-1">
          <div>
            ...
            <ColorInput value={style.backgroundColor || ''} onChange={(hex) => onStyleChange('backgroundColor', hex)} />
          </div>
          <div>
          ...
          <div>
            ...
            <Select value={style.backgroundSize || 'cover'} onValueChange={(v) => onStyleChange('backgroundSize', v)}>
              <SelectTrigger className="h-7 text-[11px] w-full" size="sm"><SelectValue /></SelectTrigger>
              <SelectContent>
          ...
          <div>
            ...
            <Select value={style.backgroundPosition || 'center'} onValueChange={(v) => onStyleChange('backgroundPosition', v)}>
              <SelectTrigger className="h-7 text-[11px] w-full" size="sm"><SelectValue /></SelectTrigger>
              <SelectContent>
      ...
      <AccordionContent>
        <div className="space-y-3 pt-1">
          ...
          <div>
            ...
            <Select value={style.borderStyle || 'none'} onValueChange={(v) => onStyleChange('borderStyle', v === 'none' ? undefined : v)}>
              <SelectTrigger className="h-7 text-[11px] w-full" size="sm"><SelectValue /></SelectTrigger>
              <SelectContent>
                <SelectItem value="none">none</SelectItem>
          ...
          <div>
            <PropLabel>Color</PropLabel>
            <ColorInput value={style.borderColor || ''} onChange={(hex) => onStyleChange('borderColor', hex)} />
          </div>
          <div>
      ...
      <AccordionContent>
        <div className="space-y-3 pt-1">
          ...
          <div>
            ...
            <Select value={style.overflow || 'visible'} onValueChange={(v) => onStyleChange('overflow', v === 'visible' ? undefined : v)}>
              <SelectTrigger className="h-7 text-[11px] w-full" size="sm"><SelectValue /></SelectTrigger>
              <SelectContent>
      ...
      <div className="p-4 space-y-4">
        {!hasSelection && (
          <div className="flex flex-col items-center justify-center py-8 text-center">
            ...
            <p className="text-sm text-muted-foreground">{t.props.noSelection}</p>
          </div>
        )}
        ...
        {hasSelection && selectedNode ? (
          <Accordion type="multiple" defaultValue={defaultAccordionValues} className="w-full">
            <ContentEditor node={selectedNode} onStyleChange={handleStyleChange} onContentChange={handleContentChange} />
            ...
            <TypographyEditor node={selectedNode} style={selectedStyle} onStyleChange={handleStyleChange} />
        ...
        {!hasSelection && (
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium">{t.props.designSystem}</span>
              {designSystem && (
                ...
                <AccordionContent>
                  <div className="grid grid-cols-2 gap-2 pt-1">
                    {DESIGN_TOKEN_COLORS.map((color) => (
                      <div key={color.name} className="flex items-center gap-2 p-2 rounded-md hover:bg-muted/50 transition-colors">
                        <div className="size-5 rounded-md border shadow-sm shrink-0" style={{ backgroundColor: color.value }} />
                        <div className="min-w-0">
                          <p className="text-[11px] font-medium truncate">{color.name}</p>
                ...
                <AccordionContent>
                  <div className="space-y-2 pt-1">
                    {TYPOGRAPHY_SIZES.map((type) => (
                      <div key={type.name} className="flex items-center justify-between p-2 rounded-md hover:bg-muted/50 transition-colors">
                        <span
                          style={{ fontSize: `${Math.min(parseInt(type.size), 20)}px`, fontWeight: type.weight }}
                          className="truncate"
                        >
                          {type.name}
                        </span>
                        <span className="text-[10px] text-muted-foreground font-mono shrink-0 ml-2">{type.size}</span>
                      </div>
                    ))}
                  </div>
                </AccordionContent>
                ...
                <AccordionContent>
                  <div className="space-y-1.5 pt-1">
                    {SPACING_SCALE.map((spacing) => {
                      ...
                      return (
                        <div key={spacing.name} className="flex items-center gap-2 p-1.5 rounded-md hover:bg-muted/50 transition-colors">
                          <div className="h-3 rounded-sm bg-emerald-500/40" style={{ width: `${Math.max(px * 2, 4)}px` }} />
                          <span className="text-[11px] font-medium min-w-[28px]">{spacing.name}</span>
                          <span className="text-[10px] text-muted-foreground font-mono ml-auto">{spacing.value}</span>
                        </div>
                      );
                    })}
                  </div>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium">{t.props.accessibility}</span>
            </div>
            <Tooltip>
              <TooltipTrigger asChild>
                <Button variant="ghost" size="icon" className="size-7 text-muted-foreground hover:text-emerald-600" onClick={() => setA11yOpen(true)}>
                  <Scan className="size-3.5" />
                </Button>
              </TooltipTrigger>
              <TooltipContent side="left">{t.a11y?.title || 'Accessibility Scanner'}</TooltipContent>
            </Tooltip>
          </div>
          <button
            type="button"
          ...
          >
            <div className={`flex items-center justify-center size-10 rounded-full ${a11yScore !== null ? getScoreBgColor(a11yScore) : 'bg-muted'}`}>
              <span className={`text-sm font-bold ${a11yScore !== null ? getScoreColor(a11yScore) : 'text-muted-foreground'}`}>
