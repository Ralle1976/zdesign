...
interface ZDesignState {
  ...
  /** Patch a node WITHOUT recording undo history (used for async generative imagery). */
  patchImageNode: (nodeId: string, updates: Partial<DesignNode>) => void;
  /** Read a node by id from the current tree (returns null if not found). */
  getNodeById: (nodeId: string) => DesignNode | null;
  deleteNode: (nodeId: string) => void;
  nudgeNode: (nodeId: string, dx: number, dy: number) => void;

  // HTML Artifact mode (agentic art-directed HTML rendered live via iframe)
  ...
  setDesignHTML: (
    ...
    scores?: { harmony: number; life: number; radiance: number; hierarchy: number; craftsmanship: number } | null,
  ) => void;
  /** Load HTML artifact from DB without marking dirty (initial load). */
  loadDesignHTML: (html: string) => void;
...
export const useZDesignStore = create<ZDesignState>((set, get) => ({
  ...
  patchImageNode: (nodeId, updates) =>
    set((state) => {
      const newTree = updateNodeInTree(state.designTree, nodeId, updates);
      // Silent: do NOT push undo history — generative imagery is a completion,
      // not a user edit (otherwise N parallel images would pollute Ctrl+Z).
      return { designTree: newTree, isDirty: true };
    }),
  getNodeById: (nodeId) => findNodeInTree(get().designTree, nodeId),
  deleteNode: (nodeId) =>
    set((state) => {
  ...
  loadDesignHTML: (html) =>
    set({ designMode: 'HTML_ARTIFACT', designHTML: html, agentTrace: [], agentScores: null, isDirty: false }),
