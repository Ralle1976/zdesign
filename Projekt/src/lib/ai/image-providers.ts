...
async function minimaxImage(prompt: string, aspect: string, apiKey: string): Promise<ImageGenSuccess | null> {
  try {
    ...
    if (!res.ok) {
      console.warn('[image-providers] minimax non-OK:', res.status);
      return null;
    }
    const data = await res.json() as {
    ...
    if (!urls || urls.length === 0) {
      console.warn('[image-providers] minimax no image_urls');
      return null;
    }
    return { url: urls[0], provider: 'minimax', model: 'image-01', free: false };
  } catch (e) {
    console.warn('[image-providers] minimax failed:', e instanceof Error ? e.message : e);
    return null;
  }
}
...
export async function generateImageWithProvider(
  ...
  // Convert pixel size to aspect ratio for Minimax (which uses "16:9" not "1024x768")
  const aspect = (() => {
    const [w, h] = size.split('x').map(n => parseInt(n, 10) || 1024);
    if (w > h * 1.3) return '16:9';
    if (h > w * 1.3) return '9:16';
    return '1:1';
  })();
  ...
  // Other configured providers as fallback — Minimax has HIGHEST priority
