...
export class ProviderRegistry {
  ...
  /**
   * Persona-aware chat for the Fusion pipeline. Resolves the provider to use for
   * a given panel role (e.g. 'ux-architect' -> Anthropic when configured, else
   * falls back through active providers, else Z.ai), applies the persona's model
   * hint, and delegates to chat() so it inherits all rate-limit/429 rotation.
   *
   * `forcedPreferredProvider` overrides the persona routing when set by the caller
   * (e.g. an explicit user choice); it is still subject to rate-limit rotation.
   */
  async routedChat(
    persona: PersonaRole,
    request: LLMChatRequest,
    forcedPreferredProvider?: string,
  ): Promise<LLMChatResponse> {
    const preferred = forcedPreferredProvider ?? resolvePersonaProvider(persona, this.getActiveProviders());
    // Apply the persona's model hint only if the caller didn't pin a model.
    if (!request.model) {
      const hint = personaPreferredModel(persona);
      if (hint) request.model = hint;
    }
    return this.chat(request, preferred);
  }
