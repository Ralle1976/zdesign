...
export function getProviderRegistry(): ProviderConfig[] {
  ...
  const providers: ProviderConfig[] = [
    ...
    // ============ IMAGE ============
    // Multi-provider image generation (2026-07): user picks the provider in
    // settings. Pollinations is the free no-key default so image gen works
    // out-of-the-box; DeepInfra/fal/Replicate are ultra-cheap paid upgrades.
    {
      id: "pollinations",
      ...
      connectionType: "api",
      enabled: true,
      active: true, // default — always available, no key required
    ...
    },
    {
      ...
      quota: "pay per image",
      status: has("DEEPINFRA_API_KEY") ? "connected" : "disconnected",
    },
    {
      id: "fal",
      name: "fal.ai",
      ...
      selectedModel: "fal-ai/flux/schnell",
      apiKeyEnv: "FAL_KEY",
      endpoint: "https://fal.run",
      quota: "pay per image",
      status: has("FAL_KEY") ? "connected" : "disconnected",
    },
    {
      id: "replicate",
      name: "Replicate (FLUX)",
