  ...
  const startWithRecording = useCallback(async () => {
    try {
      ...
      mediaRecorder.onstop = async () => {
        ...
        // Empty recording -> nothing to transcribe. Surface as a soft error
        // so the caller can react, but never crash or block the UI.
        if (audioBlob.size === 0) {
        ...
        // Whisper wants the raw audio file, so send multipart/form-data with
        // the webm Blob on the 'audio' field — NOT base64 JSON.
        try {
          ...
          if (res.ok) {
            ...
            if (transcribed) {
              ...
              // Successful call, but nothing recognized — flag it.
